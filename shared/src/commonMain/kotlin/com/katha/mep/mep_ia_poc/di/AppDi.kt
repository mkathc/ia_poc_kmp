...
import com.katha.mep.mep_ia_poc.data.localai.NativeLocalAiCapability
...
import com.katha.mep.mep_ia_poc.data.sdk.NativeSdkAdapter
import com.katha.mep.mep_ia_poc.data.sdk.NativeSdkChatService
...
object AppDi {
    ...
    private val sdkChatService: ChatService = NativeSdkChatService(
        adapter = NativeSdkAdapter(),
        configProvider = ::pocConfig,
        metricsCollector = metricsCollector,
    )
    private val localAiCapability = NativeLocalAiCapability()
    ...
    val personalizationService: PersonalizationService
        get() = if (config.useRemoteGateway) {
            ...
            FakePersonalizationService(::pocConfig)
        }
    ...
    val chatViewModel: ChatViewModel = ChatViewModel(
        ...
        getEmergencyGuideUseCase = getEmergencyGuideUseCase,
        createEmergencyReportUseCase = createEmergencyReportUseCase,
        getPendingEmergencyReportsUseCase = getPendingEmergencyReportsUseCase,
        syncPendingEmergencyActionsUseCase = syncPendingEmergencyActionsUseCase,
    ...
    val getEmergencyGuideUseCase: GetEmergencyGuideUseCase
        get() = GetEmergencyGuideUseCase(emergencySupportService)

    val createEmergencyReportUseCase: CreateEmergencyReportUseCase
        get() = CreateEmergencyReportUseCase(emergencySupportService)

    val getPendingEmergencyReportsUseCase: GetPendingEmergencyReportsUseCase
        get() = GetPendingEmergencyReportsUseCase(emergencySupportService)

    val syncPendingEmergencyActionsUseCase: SyncPendingEmergencyActionsUseCase
        get() = SyncPendingEmergencyActionsUseCase(emergencySupportService)

    val executeLocalAiCapabilityUseCase: ExecuteLocalAiCapabilityUseCase
        get() = ExecuteLocalAiCapabilityUseCase(localAiCapability, metricsCollector)
