...
actual class NativeSdkAdapter actual constructor() : ChatProviderAdapter {
    private val sdk = FakeNativeChatSdkIos()
...
private class FakeNativeChatSdkIos {
    suspend fun sendMessage(
        ...
        val messageId = "ios-sdk-${text.hashCode()}"
        onStarted(messageId)
        when (scenario) {
            ...
            FakeGatewayScenario.timeout -> {
                delay(3_500L)
                val tokens = listOf("Respuesta ", "tardía ", "desde SDK iOS.")
                tokens.forEach {
                ...
                val tokens = listOf("Respuesta ", "desde ", "SDK iOS.")
                tokens.forEach {
                    delay(180L)
                    onToken(messageId, it)
                }
